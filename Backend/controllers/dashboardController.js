...
// Get admin dashboard stats (admin only) - Optimized to single query logic
exports.getAdminStats = async (req, res) => {
    try {
        // Use RPC or parallelize if RPC not available. 
        // For now, let's use Promise.all to fetch in parallel as a mid-step improvement 
        // since we don't have access to create RPCs in Supabase directly here.
        const [
            { count: totalReports },
            { count: pending },
            { count: inProgress },
            ...
            supabase.from("reports").select("*", { count: "exact", head: true }).eq("priority", "high"),
            supabase.from("reports").select("*", { count: "exact", head: true }).eq("priority", "low"),
        ...
        res.json({
            ...
            pending: (pending || 0) + (inProgress || 0), // Count all non-completed/non-rejected as workload/pending for summary
            completed: completed || 0,
            collectors: collectors || 0,
            // Original fields for internal use if needed
            pending_raw: pending || 0,
